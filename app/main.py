...
def load_config(config_path):
    ...
    if config.has_section("common"):
        ...
        if "auto_select_all" in common_config:
            common_config["auto_select_all"] = str_to_bool(common_config["auto_select_all"])
        # 默认跳过未开放章节，不重试
    ...
    if config.has_section("tiku"):
        ...
        if tiku_config.get("provider", "").strip() != "AI":
...
def init_chaoxing(username, password, tiku=None):
    """初始化超星实例"""
...
def build_tiku(tiku_config):
    """根据配置构建题库实例"""
    base_tiku = Tiku()
    if tiku_config:
        base_tiku.config_set(tiku_config)

    tiku = base_tiku.get_tiku_from_config()
    try:
        tiku.init_tiku()
    except Exception as exc:
        logger.warning(f"题库初始化失败，已自动禁用: {exc}")
        tiku = Tiku()
        tiku.DISABLE = True

    if tiku.DISABLE:
        logger.info("题库已禁用，章节测验将跳过")
    else:
        logger.info(f"题库已启用: {tiku.name}")
...
def select_courses(all_courses, course_ids=None):
    """选择要学习的课程"""
    if course_ids and "*" in course_ids:
        return all_courses
...
def main():
    ...
    try:
        ...
        if config_file:
            common_config, tiku_config, _notification_config = load_config(config_file)
        ...
        # 初始化题库与超星实例
        tiku = build_tiku(tiku_config)
        chaoxing = init_chaoxing(username, password, tiku)
