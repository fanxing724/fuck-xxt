...
class Chaoxing:
    ...
    def get_job_list(self, _clazzid, _courseid, _cpi, _knowledgeid, _job_count=None):
        _session = init_session()
        job_list = []
        job_info = {}
        seen_jobs = set()
        ...
        try:
            max_cards = max(1, int(_job_count)) if _job_count is not None else 7
        except (TypeError, ValueError):
            max_cards = 7

        for _possible_num in range(max_cards):
