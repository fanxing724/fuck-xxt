...
class AI(Tiku):
    ...
    def _init_tiku(self):
        self.endpoint = self._conf.get('endpoint') or self._conf.get('openai_api_base') or 'https://api.openai.com/v1'
        self.key = self._conf.get('key') or self._conf.get('openai_api_key')
        self.model = self._conf.get('model') or self._conf.get('openai_model') or 'gpt-4o-mini'
        self.http_proxy = self._conf.get('http_proxy') or self._conf.get('proxy')
        self.min_interval_seconds = int(self._conf.get('min_interval_seconds', 3))

        if not self.key:
            raise KeyError("AI题库缺少 api key")
